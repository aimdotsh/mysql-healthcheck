...
// 单连接私有 buffer 峰值（MB）：sort + join + read + read_rnd + tmp_table
// 这些 buffer 是「每连接」分配的，高并发下会叠加吃内存（与 buffer pool 这类全局内存相对）。
...
function evalDualMasterBothWritable(ctx) {
  ...
  // 只从 IP 排序较小的那端出一条 issue，避免两端各出一条重复
