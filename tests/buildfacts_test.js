...
assert.strictEqual(facts.issues.length, 50, `expected 50 issues, got ${facts.issues.length}`);
const byP = p => facts.issues.filter(i => i.priority === p).length;
assert.deepStrictEqual([byP('P0'), byP('P1'), byP('P2'), byP('P3')], [5, 11, 29, 5], 'priority distribution drift');
