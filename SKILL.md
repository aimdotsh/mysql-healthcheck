...
- 默认输出文件名：`<项目名>_MySQL数据库巡检报告_详细版_v3.1.docx`，与 data.json 同目录。
...
```
<项目名>_MySQL数据库巡检报告_详细版_v3.1.docx
```

例：`一卡通_MySQL数据库巡检报告_详细版_v3.1.docx`
...
`mysql-inspection-report`（8 章 v1.0）保留原状，适用于快速概览。本技能（13 章 v3.1）适用于深度技术审计。两者数据来源相同，但本版表达更详尽。

---

## 版本

- v3.1（当前）：数据/视图分离 + 自动分析 + 多节点自适应 + 占位符校验
- v2.0：13 章模板，硬编码占位
- v1.0：8 章精简版
