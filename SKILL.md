...
description: 为 MySQL 数据库集群生成商业可交付级巡检报告（.docx，内嵌 10 张图表）。当用户提供包含 MySQLHealthCheck_*.txt 的数据目录并要求「生成巡检报告」「整理巡检数据」「写月度巡检」「健康评估」「上线评估」「故障复盘」「合规自查」「商业交付级报告」等任务时使用。本版（ykt 2.0）走 Node 脚本链路（extract.js → render.js）直出 docx，无需 LLM 介入逐章撰写。产物含 17 章详细分析 + 执行摘要 + 自动目录 + 六维度健康度评分 + TOP SQL 治理 + 备份评估 + 等保/PCI/GDPR/SOX 合规对照表 + 10 张嵌入图表（健康度仪表 / 六维雷达 / 问题分布 / 拓扑 / 磁盘 / 连接 / Processlist / Buffer Pool / TOP10 大表 / 合规分布）。
---

# MySQL 巡检报告 Skill（ykt 2.0 · Node 直出 docx）

> 本版用 **Node 脚本链路**（`extract.js` → `render.js`）确定性地生成 docx 报告，
> **图表由 `lib/charts.js` 用 SVG → PNG 直接渲染并嵌入**，全程不依赖 LLM 逐章撰写。
> 同 txt 必出同 docx，适合商业交付 / 合规审计 / 批量自动化场景。
...
> 下文 `<skill>` 指本 skill 解压后的根目录（含 `SKILL.md` / `scripts/` / `collectors/`）。
...
- `--report-version <x.y>` 可选 —— 报告版本号（封面/页眉/文件名，默认 `2.0`）
- `--out <path.json>` 可选 —— 自定义 data.json 输出位置（默认写入数据目录）
- `--config <path>` 可选 —— 外部规则配置（见 Step 0）
...
### Step 2：渲染 docx（data.json → docx + 10 张图表）
...
- `<data.json>` 必填
- `--out <path.docx>` 可选
- `--soffice <path>` 可选 —— 指定 LibreOffice 二进制，导出即带完整目录页码（缺省时目录留空，需在 Word/WPS 右键更新域）
...
**一步到位**（extract + render 串联）：
```bash
node build.js <数据目录> --project "<项目正式名>"
```
...
ykt 2.0 走 Node 链路，需要一次 `npm install`：

```bash
cd <skill>/scripts
npm install        # 装 docx@^8.5.0 + @resvg/resvg-js@^2.6.2
```

依赖：Node.js ≥ 16；npm；`docx@^8.5.0`（生成 docx）；`@resvg/resvg-js@^2.6.2`（SVG 图表转 PNG 嵌入）。

也可直接跑 `bash install.sh`（封装了上面的 npm install）。

---

## 内嵌图表（10 张）

`render.js` 通过 `lib/charts.js`（SVG → PNG）按数据可用性自动嵌入下列图表：

| # | 图表 | 类型 | 章节 |
|---|---|---|---|
| 1 | 集群健康度仪表 | gauge | 执行摘要 |
| 2 | 六维度雷达 | radar | 执行摘要 |
| 3 | 问题分布（P0~P3） | pie | 第一章 |
| 4 | 集群拓扑 | topology | 第二章 |
| 5 | 磁盘使用率 | hbar | 第二章 |
| 6 | 连接数分布 | hbar | 第三章 |
| 7 | Processlist 命令分布 | pie | 第七章 |
| 8 | Buffer Pool 命中率 | vbar | 第八章 |
| 9 | TOP 10 大表 | hbar | 第七章 |
| 10 | 合规检查分布 | pie | 第十六章 |

> 某图表所在章节无数据时会自动跳过（如该节点未采集到 processlist），属正常行为。
...
```
<项目名>_MySQL健康巡检报告_v2.0.docx
```

项目名中的特殊字符会被替换为 `_`；版本号取 `--report-version`（默认 `2.0`）。
...
| `错误：未找到可用的 docx 依赖` | 漏装 npm 依赖 | `cd <skill>/scripts && npm install` |
| `图表生成失败` / 图表占位为空 | `@resvg/resvg-js` 未装或装坏 | `npm install` 重装；不影响报告正文，仅图表缺失 |
| 某节点字段全是 `-` | txt 段名与解析器期待不一致 | 查 `references/parsing.md`，必要时改 `extract.js` 中的 `getSection()` 关键字 |
| 目录页码空白 | 未装 LibreOffice | 在 Word/WPS 右键目录 → 更新域；或 `--soffice <path>` 导出即带页码 |
...
- [ ] 命令行输出 `✓ schema 校验通过` + `✓ 占位符校验通过`
- [ ] docx 17 章齐全，含执行摘要 + 目录
- [ ] 封面显示项目名、巡检日期、拓扑摘要、`v2.0版`
- [ ] 执行摘要的健康度仪表 + 六维雷达图正常显示
...
```
mysql-healthcheck/                  # ykt 2.0 解压根目录
├── SKILL.md                        # 本文档（agent 协议）
├── README.md                       # 仓库介绍 + 快速上手
├── USAGE.md                        # 完整使用说明（人类视角）
├── CHANGELOG.md                    # 版本变更
├── VERSION                         # 2.0.0（程序可读版本号）
├── install.sh                      # 一键 npm install
├── references/                     # 按需加载的技术细节
│   ├── visual-spec.md
│   ├── rules.md / rules-auto.md
│   ├── parsing.md
│   └── interview-guide.md
├── collectors/
│   └── mysqlHealthCheckV3.0.sh     # 纯 bash 采集脚本
└── scripts/
    ├── extract.js                  # txt → data.json（含规则引擎）
    ├── render.js                   # data.json → docx
    ├── build.js                    # extract + render 串联
    ├── lib/charts.js               # SVG → PNG 图表
    ├── lib/schema.js               # data.json schema 校验
    ├── rule-engine.js              # 声明式规则引擎
    ├── rule-helpers/index.js       # 规则 handler 原语库
    ├── rules/*.json                # 42 条规则（6 维度）
    ├── config/                     # 默认阈值 + 样例
    └── assets/logo.png
```
