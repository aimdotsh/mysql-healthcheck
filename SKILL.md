...
> 2. **保留 17 章的位置**：即使在 standard / fast 模式下省略某章，**保留章节的位置感** —— 比如 fast 模式只输出第 1 / 16 / 17 章，**仍用「第一章」「第十六章」「第十七章」编号**，而不是「第 1 / 2 / 3 章」重新顺序编号
> 3. **章节标题用中文数字**（一/二/三 ...），与模板一致；不要用阿拉伯数字
> 4. **省略某章时**：standard 模式跳过无数据章节即可（**不写**，**不留占位**，**不重新编号**），但必填章节（1/4/5/12/16/17）哪怕数据少也要写
> 5. **不能自创新章节** — 如果数据触发了模板没覆盖的话题，放进 16 章「行动计划」或 16 章后加一个 16.4 小节，不要单独开新章
>
...
| 模式 | 必填章节 | 可选省略 |
|---|---|---|
| **fast** | 第一章 / 第十六章 / 第十七章 | 第二至十五章全部 |
| **standard** | 第一/二/四/五/六/十二/十三/十四/十五/十六/十七章 | 第三/七/八/九/十/十一章（无数据时省略） |
| **full** | 全 17 章 | 无（必须全部输出） |

**示例：standard 模式 fast 实测样本（4 节点集群）**：

```
第一章 执行摘要
第二章 操作系统与硬件
第四章 集群拓扑           ← 注意：跳过了第三章，但保留"第四章"编号
第五章 关键参数与一致性
第六章 性能指标分析
第十二章 主从复制         ← 跳过 7-11
第十三章 Schema 审计
...
如果用户问"为什么少了第三章/第七章"，回答："standard 模式下该章无可报告数据，已省略；如需完整报告请用 full 模式"。
