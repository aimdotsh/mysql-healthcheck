...
> 首页和采集脚本介绍页面风格不统一，把采集脚本介绍页面修改长根首页相同的风格，亮丽色。

### 问题

v5.0.6 新增的 `/collector.html` 用了独立的暗色 banner header（深蓝 → 蓝紫渐变），与首页风格"白卡 + 浅灰渐变底"完全不一致：

| 元素 | 旧 collector.html | 首页 index.html |
|---|---|---|
| body 背景 | 纯灰 `#f5f7fa` | 浅灰渐变 `linear-gradient(135deg, #f5f7fa, #e6ebf1)` |
| header 区 | 深蓝渐变 banner（暗） | 纯文字 h1 + subtitle（亮） |
| 卡片 | section.card，自定义样式 | .card，白底 12px 圆角 |
| 按钮主色 | 绿色 `#059669` | 深蓝 `#1f4e79` |
| 表格 / 代码块 | 各自样式 | 一致设计 |

### 改动

完全重写 `saas/public/collector.html` 的 CSS，全部直接复用首页的设计语言：

1. **body 背景**：改成首页同款 `linear-gradient(135deg, #f5f7fa 0%, #e6ebf1 100%)`
2. **header**：去掉暗色 banner，改成首页风的纯文字 h1（`#1f4e79`）+ subtitle，**新增面包屑导航**（首页 › 采集脚本）
3. **卡片**：白底 12px 圆角 28px 内边距 + 同样的阴影 `0 4px 6px rgba(0,0,0,0.04)`，与首页 `.card` 完全一致
4. **下载按钮**：从绿色 `#059669` 改成首页主色 `#1f4e79`，hover `#163a5a` — 视觉强调由按钮**形状**和**位置**而非颜色突兀来达成
5. **下载卡背景**：保留淡绿渐变（亮丽感）但更柔和，匹配整体浅色调
6. **表格**：表头蓝色 `#1f4e79`，hover 行 `#fafbfc`，与首页参数差异表样式一致
7. **代码块**：浅灰底（`#f9fafb` + 边框）替代之前的暗黑底，与首页 textarea 风格一致
8. **提示框**：`.note-tip` 蓝色信息条 + `.note-warn` 橙色警告条（复用首页 `.api-key-bar` 的橙色调）
9. **标签 chip**：复用首页 `.badge-single / .badge-cluster` 同款配色

### 验证

```bash
curl -s http://127.0.0.1:3000/ | grep -c "#1f4e79"
# 多次出现，主色

curl -s http://127.0.0.1:3000/collector.html | grep -c "#1f4e79"
# 多次出现，与首页一致

# 背景渐变两边一致：
首页 / 采集页：linear-gradient(135deg, #f5f7fa 0%, #e6ebf1 100%)
```
...
> 不能出现零分和很低的分数 — 数据库实际在跑，0/100 / 30/100 这种分数会吓到客户

### 旧模型问题

- P0 = -18, P1 = -7 — 单条 P0 issue 直接砍掉一个维度近 1/5 分
- 多条同类 issue 线性堆叠：3 条 P0 = -54 = 维度 46/100
- 最低分无下限，理论上可以 0 分
- 备份 / 安全二次惩罚 -15 每项，且和 issues 重叠扣分（双重惩罚）

### 新评分模型

1. **单条惩罚减半**：P0: 18→8, P1: 7→3, P2: 3→1.5, P3: 1→0.5
2. **递减惩罚**（同维度同优先级多条 issue 不再线性叠加）：
...
3. **每维度下限 50 分**（不再 0 分）
4. **总分下限 55 分**（数据库在跑的事实应该反映在评分里）
5. **二次惩罚减半**：backup tool / dir 各 -15→-4；TLS / encryption -5→-2；audit -3→-1（避免与 issue 重复扣分）
...
完整新分段：≥ 90 优秀 / ≥ 80 良好 / ≥ 70 中等 / ≥ 60 关注 / < 60 建议优化

### 实测对比

| 样本 | 旧分（估算） | 新分 |
|---|---|---|
| 2021 单点（10 issues, 2 P0 + 2 P1）| ~75 | **95/100** |
| v3 4 节点集群（68 issues, 7 P0 + 19 P1）| ~50 | **86/100** |
| 全清空 (0 issues + 满配置) | 100 | 100 |
| **极端**（每维度 10 条 P0 = 60 条 P0）| ~0 | **73/100**（floor 生效）|

### 实现

- `computeHealthScore()` 完全重写（保持函数签名 + 返回结构兼容）
- `levelOf()` 重写分段标签
- 测试同步：`tests/report_regression_test.js` 中健康度断言（如有）不变 —— v3 样本从未达 < 55 的分数，新模型只是把分数普遍抬高几分

---

## [4.9.3] - 2026-05-18

**报告措辞咨询性重写**。客户反馈：生产数据库一直在正常运行，但巡检报告大量使用「P0 紧急 / 立即处理 / 立即修复 / 数据丢失风险 / 必须立即收紧」等急迫词汇，会让客户误以为「生产环境着火」。本期把全部 30+ 处 alarmist 措辞改为**专业咨询性**风格，保留风险等级与紧迫程度，但用建议性而非命令性语气。
...
|---|---|---|
| 优先级标签 | `P0 紧急` | **`P0 关键`** |
| 整体评估 | 「存在紧急风险，需立即处理」 | **「运行整体稳定，识别出关键风险点，建议优先关注与处置」**|
| 整体评估 | 「总体平稳，存在需短期处理的重点问题」 | **「运行整体稳定，存在若干重要风险点，建议近期规划处置」**|
| 行动计划标题 | 「🔴 本周内（P0 紧急）」 | **「🔴 优先处置（P0 关键风险点）」**|
| 行动计划标题 | 「🟠 两周内（P1 重要）」 | **「🟠 近期规划（P1 重要风险点）」**|
| Action 文本 | 「立即清理日志/历史数据 或 扩容」 | **「建议优先清理日志 / 历史数据，或评估扩容」**|
| Action 文本 | 「立即收紧 ... 立即关闭 ... 立即开启 ... 立即安装 ...」 | **「建议优先收紧 / 建议尽快关闭 / 建议开启 / 建议优先安装 ...」**|
| 安全建议 | 「⚠️ 必须立即收紧 N 个高风险账号」 | **「⚠ 建议优先收紧 N 个高风险账号」**|
| Wildcard 等级 | `🔴 致命` | **`🔴 关键`** |
| 自增列扩容提示 | 「使用率超过 80% 应立即扩容」 | **「使用率超过 80% 建议优先扩容」**|
| 自增列标识 | `🔴 紧急` | **`🔴 关键`** |
| 备份评估 | 「最近备份已 X 天前，存在数据丢失风险」 | **「最近备份已 X 天前，建议尽快重新执行全量备份并核实异地保存」**|
| 磁盘健康标签 | `紧急` | **`关键`** |
| Action plan hint | 「P0=立即处理（影响可用性），P1=本周内处理...」 | **「P0=优先处置（潜在影响可用性），P1=近期规划处置，P2=本月内评估优化，P3=持续观察」** |

### 设计原则

- **保留紧迫程度**：P0/P1/P2/P3 优先级代码不变，只换标签词
- **保留风险等级**：「关键 / 重要 / 建议 / 观察」依次递减
- **去除命令式**：所有「立即」/「马上」/「必须」改为「建议优先」/「建议尽快」/「建议」
- **去除恐慌式**：「紧急」/「致命」改为「关键」；「数据丢失」改为「持久化保障不足」/「建议尽快备份」
- **保留事实描述**：技术风险类型（如「数据漂移」「数据完整性」）仍如实描述，因为它们说明风险类型而非催促行动
- **整体评估开头加肯定**：「运行整体稳定」放在前面，承认数据库正常运行的事实，再描述风险点

### 🔧 实现

- **`scripts/extract.js`**：`deriveOverallAssessment` 4 句重写；rule actions 8 处「立即」替换；备份评估文案重写；root@% / wildcard / doublewrite / slave_skip_errors / sync_binlog 等 action 全部软化
- **`scripts/render.js`**：`priorityLabel` P0 → "关键"；`diskHealthLabel` → "关键"；wildcard 分类 → "关键"；行动计划标题；hint 文案；自增列标识
- **`tests/report_regression_test.js`**：备份断言同步新文案

### 📊 示例对照（同一个数据集生成的同一份报告）

**v4.9.2 报告开头**：
> 整体评估：**存在紧急风险，需立即处理**（健康度评分 85/100）

**v4.9.3 报告开头**：
> 整体评估：**运行整体稳定，识别出关键风险点，建议优先关注与处置**（健康度评分 85/100）

**v4.9.2 第十六章 16.2 标题**：「🔴 本周内（P0 紧急）」「🟠 两周内（P1 重要）」

**v4.9.3 第十六章 16.2 标题**：「🔴 优先处置（P0 关键风险点）」「🟠 近期规划（P1 重要风险点）」

### 🧪 验证

- `npm test`（collector_autodiscovery + report_regression）全绿
- 实测样本：所有 alarmist 词（紧急/立即/马上/必须立即/致命/本周内/两周内）已全部替换，没有遗漏
- 新词频次：「P0 关键」5 处、「优先处置」2 处、「近期规划」2 处、「建议优先」6 处、「运行整体稳定」3 处
