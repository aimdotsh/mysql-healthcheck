...
- 可选使用**当前智能体或 OpenAI-compatible API**二次研判，补充跨指标建议并识别候选规则；不改变确定性评分
...
| 🎯 **55 条巡检规则** | P0/P1/P2/P3 自动分级，跨节点同类问题智能聚合，含连接容量、当前锁等待、弱密码和参数合理性校验 |
...
| `scripts/extract.js` | 解析端 | 段落解析 + 55 条规则 + 健康度评分 + 关联推断 + 备份评估 + 安全合规 |
...
> 本项目的确定性规则库已覆盖 DBA 日常巡检核心场景；后续通过“规则基线 + 大模型候选规则”机制持续补齐可验证的规则缺口。
